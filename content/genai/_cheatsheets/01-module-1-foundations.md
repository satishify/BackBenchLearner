...
- **Backprop** = chain rule through the graph. **Gradient descent**: `θ ← θ − η ∇L`.
...
- Scores: `softmax(QKᵀ / √d_k) V`. Scale by `√d_k` so softmax stays sharp as `d` grows.
...
- **Causal mask** — position `i` cannot attend to `> i` (autoregressive next-token training).
...
- Temperature ↑ → flatter distribution (more random). `top-p` keeps smallest set of tokens with cumulative prob ≥ p.
