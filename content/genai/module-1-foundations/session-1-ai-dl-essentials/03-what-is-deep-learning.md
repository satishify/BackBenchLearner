...
- **Depth** means many stacked layers, each doing a linear transform plus a nonlinearity.
- Early layers pick up **simple patterns**; deeper layers combine them into **higher-level concepts**.
- Without nonlinearities, stacking layers collapses to one big linear map — depth would buy nothing.
- Most GenAI building blocks (transformers, diffusion models) are deep networks.
...
- **Wrong modality choice.** Flattening images into classical features or ignoring sequence structure throws away the inductive bias that makes deep learning work.
...
- **Backpropagation** — algorithm for computing gradients of the loss with respect to weights
